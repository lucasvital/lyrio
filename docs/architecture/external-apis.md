...
- **Auth:** Secret API key as Bearer (env: `REVENUECAT_API_KEY`, `REVENUECAT_PROJECT_ID`).
- **Data ingested (FR6):** subscribers, entitlements, transactions; revenue metrics (MRR, active subs, churn, trials, conversion).
- **Pagination:** cursor based; persist in `sync_state`.
- **Rate limits:** respect 429 + backoff.
- **Server-side only.**

> Confirm v1 vs v2 endpoint coverage during Story 3.1 (some metrics live in different endpoints/overview API). Use official docs via context7.
